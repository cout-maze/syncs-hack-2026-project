...
- **Cities CRUD**: scoped to `ownerId` from JWT (404, not 403, for other users' cities). New city = 30×30 grid, budget 100, no blocks. (Bumped up from the original 10×10 so the map feels like a real, explorable city rather than a small board - the block budget is what keeps a layout meaningful, not the grid size. The backend just needs the new default; nothing about the contract changed.)
